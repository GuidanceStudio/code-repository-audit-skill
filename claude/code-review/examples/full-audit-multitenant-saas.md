...
- 🟡 `app/Services/AbstractGatewayClient.php:1-67` — single concrete implementation (`AcmeGatewayClient`); abstract class adds indirection without test seam. _Suggested fix_: inline the abstract class.
...
#### 🔴 `gateway/src/acme_gateway/db_pool.py:42` — async-pool tenant context not re-applied on connection re-use.
...
- **What**: `acme-acp` repo has no `.github/workflows/` running tests, lint, or type checks.
...
- **Suggested fix**: Add `templates/ci-github-actions.yml` § lint-typescript to the acme-acp repo. ~2 h.

- 🟡 `acme-core/.github/workflows/audit.yml` runs only PHP linters; Python ruff + mypy not invoked.
...
This is the strongest dimension in the audit. Cluster-internal secrets all auto-bootstrap (the four `ACME_*_SECRET` placeholders from a previous audit cycle have been removed from `.env.example` and replaced with the named-volume bootstrap pattern). 
...
| 🔴 D8-1 | CI-3: GitHub Actions for acme-acp (lint + tsc + test) | poc.md | 2 h |
| 🟡 D1-1 | OPT-50: extract `AgentTemplateCloning` from `AgentTemplateService` | v0.x.md | 4 h |
| 🟡 D8-2 | CI-4: add ruff + mypy + Semgrep to acme-core CI | poc.md | 2 h |
