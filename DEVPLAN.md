...
Archived: M10-M30 -> `DEVPLAN-ARCHIVE.md` (one line per milestone; the sha is the pointer to the full detail). M1-M9 stay here — their headings carry no done marker.
...
empirically (detect-stack miss on acme-core, milestone-ID ordering,
...
fixes to green (6/6). Done-when verified: detect on acme-core emits
...
- [x] **Recursive stack detection.** `scripts/_detect_stack.py` only
  ...
  (verified: on `acme-core` it returns `docker shell` and misses
...
Exit gate: `_detect_stack.py` on `acme-core` emits
...
- [x] **pytest suite** (`tests/` + fixture mini-repos):
  - `_detect_stack.py`: root markers, nested monorepo (the
    acme-core regression from M1), no markers (exit 2).
...
## M31: Archive the closed milestones (M10-M30) ✅

**Why:** The devplan had grown to ~8,400 words, most of it immutable closed work (v0.3 packaging, the Ponytail integration, the token-essentiality pass) nobody rereads; compressing it to sha pointers reduces the file to whatever is still live.

**Approach:** For every milestone whose heading carries `✅` and whose tasks are all `- [x]` (M10-M30 — M1-M9 excluded, their headings carry no done marker), re-derive the shipping sha from the *current* `git log` (never from sha strings in the devplan prose — this repo's history was flagged as force-rewritten today). Several milestones ship inside a commit whose message is about something else (M17-19 inside a nominal repo-rename commit `a4f9b21`; M20/M22-26 inside `252c512`, the "v0.4 token essentiality" commit); the archive line points to the shipping commit, not the later devplan-only commit that only ticked the checkboxes. Non-milestone structure (Phase/version/Follow-up headers, their intros, "Recommended order" notes) is dropped once every milestone under it is archived, since it no longer points at anything in the active file; `## Out of scope for v0.3` is kept as-is since it is a standalone scope decision, not milestone detail.

**Tasks:**
- [x] Enumerate closed milestones (heading carries `✅` + all tasks `- [x]`); confirm M1-M9 don't qualify (no heading marker).
- [x] Re-derive each milestone's shipping sha from current `git log` (grep + `git show --stat` to tell a shipping commit from a devplan-only one), verify with `git cat-file -e`.
- [x] Write `DEVPLAN-ARCHIVE.md` with one line per milestone.
- [x] Replace the archived blocks in `DEVPLAN.md` with a pointer line; drop the now-empty Phase/version/Follow-up headers; keep `Out of scope for v0.3`.
- [x] Verify the ID set before archiving equals active + archive after (script diff).
- [x] Run `python3 -m pytest tests/ -q` and confirm still green.
- [x] Commit devplan + archive together, push to `main`.

**Done when:** `DEVPLAN.md` contains M1-M9, `Out of scope for v0.3`, and this milestone; `DEVPLAN-ARCHIVE.md` contains 21 verified lines; the ID-set diff is empty; pytest is green.

**Execution notes:** 21/21 milestones archived (0 without a findable commit). ID-set diff before/after: empty (M31 is the only new ID).
